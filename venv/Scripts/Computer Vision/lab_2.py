...
def load_data():
    import os
    apple_path = os.listdir(r'dataset\zebra')
    banana_path = os.listdir(r'dataset\banana')

    apple = []

    for i in apple_path:
        img = cv2.imread(r"dataset\\zebra\\" + i, 0)
...
def load_test_data():
    import os
    apple_path = os.listdir(r'dataset\zebra_test')
    banana_path = os.listdir(r'dataset\banana_test')

    apple = []

    for i in apple_path:
        img = cv2.imread(r"dataset\\zebra_test\\" + i, 0)
        img = cv2.resize(img, (500, 500))
        apple.append(img)
